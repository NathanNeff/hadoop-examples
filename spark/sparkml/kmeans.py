...
# Load the data from a directory with device status data including
# latitude, longitude
...
# Create a vector assembler that will take in our DataFrame and convert the
# inputCols specified
va = VectorAssembler(inputCols=["lat","lon"],outputCol="features")

# Use the vector assembler to transform the DataFame which will add a new
# column called 'features' which will be of the Vector type

vectorDF = va.transform(latLonDF)

# Create a Kmeans estimator that takes the "features" column as input and set
# the value for K to 5 with a tolerance of .01 and a seed # of 12345
